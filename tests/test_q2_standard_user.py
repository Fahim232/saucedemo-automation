...
@allure.title("Q2 - standard_user full purchase journey")
@allure.feature("Purchase journey")
@pytest.mark.q2
def test_standard_user_full_purchase_journey(driver):
    with allure.step("Log in with standard_user"):
    ...
    with allure.step("Add three items to the cart"):
        for item in ITEMS:
    ...
    with allure.step("Verify product names on the final checkout page"):
        assert checkout_page.get_item_names() == ITEMS
